...
ax.plot(t,normalize(response_values),ls='solid',marker='',color='red',lw=2.9,label="model's response",ms=2.5)
ax.plot(circ_t,normalize(circ_filtered),ls='solid',marker='',color='green',lw=2.9,label='Mah et al. response',ms=0.5)
ax.plot(t,normalize(NTSI_filtered),ls='dashed',marker='',color='black',lw=2.0,label="NTSI")

for ticklbl in ax.get_xticklabels()+ax.get_yticklabels():
    ticklbl.set_fontsize(15)

ax.set_ylabel('Normalized response/stimulus',fontsize=17)
