...
## read data
path_prefix = "/media/windows/ilyas/data/tuning_curves/L4_network/ommatidia_equatorial/runtime_35s/grating/"
...
for i in range(len(ac_objects)):
    ac_i = ac_objects[i]
    ac_i.plot_mean_resp(ax_grating_tuning_L, 'left tangential HS',
                        plot_kwargs=ac_plots_kwargs[i], min_max_speeds=[-360, 360])
    ac_i.plot_mean_resp(ax_grating_tuning_R, 'right tangential HS',
                        plot_kwargs=ac_plots_kwargs[i], min_max_speeds=[-360, 360])
